...
const NumberInput = ({ value, onChange, className }) => {
  ...
  return <input ref={inputRef} type="text" inputMode="numeric" defaultValue={value} onBlur={handleBlur} onKeyDown={handleKeyDown} className={className} />;
};

const QuickSelectWithInput = ({ label, value, onChange, themeColor = "#2563eb" }) => (
  <div>
    <label className="block text-xs font-medium text-gray-600 mb-1">{label}</label>
    <div className="flex items-center gap-1">
      <NumberInput value={value} onChange={onChange} className="flex-1 px-2 py-1.5 border rounded text-center text-sm font-bold focus:ring-2" style={{ "--tw-ring-color": themeColor }} />
    </div>
  </div>
);
...
const StatCard = ({ value, label, bgColor = "bg-gray-50", textColor = "text-gray-600" }) => (
  ...
  <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
    <div className={`bg-gradient-to-r ${SECTION_COLORS[color]} px-4 py-2.5`}>
      <h3 className="text-base font-semibold text-white flex items-center gap-2">
        <span className="bg-white text-gray-700 w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold">{text}</span>
        <span>{icon}</span>
        {title}
      </h3>
    </div>
...
const InfoBox = ({ children, color = "blue", className = "" }) => {
  ...
  return <div className={`p-2 rounded-lg border text-xs ${colorMap[color]} ${className}`}>{children}</div>;
};

const CheckboxOption = ({ checked, onChange, label, sublabel, isActive }) => (
  <label className={`flex items-center gap-2 p-2 rounded-lg border-2 cursor-pointer transition-all ${isActive ? "bg-yellow-50 border-yellow-400" : "bg-gray-50 border-gray-200"}`}>
    <input type="checkbox" checked={checked} onChange={onChange} className="w-4 h-4 text-yellow-500 rounded" />
    <div>
      <span className="text-xs font-medium">{label}</span>
      {sublabel && <span className="text-xs text-yellow-600 ml-1">{sublabel}</span>}
    </div>
  </label>
);

const RadioOptionCard = ({ name, checked, onChange, label, sublabel, isActive }) => (
  <label className={`flex flex-col items-center gap-1 p-3 rounded-lg border-2 cursor-pointer transition-all ${isActive ? "bg-red-50 border-red-400" : "bg-gray-50 border-gray-200 hover:border-gray-300"}`}>
    <input type="radio" name={name} checked={checked} onChange={onChange} className="w-4 h-4 text-red-500" />
    <span className="text-xs font-medium">{label}</span>
    {sublabel && <span className="text-[10px] text-red-500 font-bold">{sublabel}</span>}
  </label>
);
...
const Modal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4" onClick={onClose}>
      <div className="relative bg-white rounded-lg shadow-2xl max-w-[95vw] max-h-[95vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-2 right-2 z-10 bg-red-500 hover:bg-red-600 text-white rounded-full w-10 h-10 flex items-center justify-center text-xl font-bold shadow-lg transition-colors">
          ✕
        </button>
        <div className="p-4">{children}</div>
      </div>
...
const DimLine = ({ x1, y1, x2, y2, value, offset = 25, vertical = false, color = "#000", fontSize = 9, unit = "" }) => {
  const arrowSize = 3;
  ...
  if (vertical) {
    ...
    return (
      <g className="dimension">
        ...
        <rect x={lineX - textWidth / 2} y={midY - 6} width={textWidth} height="12" fill="white" />
  ...
  return (
    <g className="dimension">
      ...
      <rect x={midX - textWidth / 2} y={lineY - 6} width={textWidth} height="12" fill="white" />
...
const CenterLine = ({ x1, y1, x2, y2 }) => <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#000" strokeWidth="0.3" strokeDasharray="10,3,2,3" />;

const HatchLines = ({ x, y, width, height, count, color = "#ddd", direction = "diagonal" }) => (
  <>
    {[...Array(count)].map((_, i) => (
      <line key={i} x1={x + 1} y1={y + i * (height / count)} x2={x + width - 1} y2={y + i * (height / count) + 4} stroke={color} strokeWidth="0.2" />
    ))}
  </>
);
...
const LockBlockSVG = ({ x, y, width, height, strokeColor = "#c62828" }) => (
  <g>
    <rect x={x} y={y} width={width} height={height} fill="#ffcdd2" stroke={strokeColor} strokeWidth="0.8" />
    <line x1={x} y1={y} x2={x + width} y2={y + height} stroke={strokeColor} strokeWidth="0.4" />
    <line x1={x + width} y1={y} x2={x} y2={y + height} stroke={strokeColor} strokeWidth="0.4" />
  </g>
);
...
const EngineeringDrawing = ({ results }) => {
  const { W, H, T, S, F, DF, R, totalFrameWidth, railPositions, railSections, lockBlockTop, lockBlockBottom, lockBlockLeft, lockBlockRight, lockBlockPosition, lockBlockCount, currentFrame, doubleFrame } = results;
  ...
  const renderDoubleFrames = () => {
    ...
    if (doubleFrame.left) {
      for (let i = 0; i < doubleFrame.count; i++) {
        elements.push(<rect key={`df-left-${i}`} x={positions.front.x + dims.front.F + dims.front.F * i} y={positions.front.y + dims.front.F} width={dims.front.F} height={dims.front.H - 2 * dims.front.F} fill="rgba(255, 143, 0, 0.1)" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />);
      }
    }
    ...
    if (doubleFrame.right) {
      for (let i = 0; i < doubleFrame.count; i++) {
        elements.push(<rect key={`df-right-${i}`} x={positions.front.x + dims.front.W - dims.front.F - dims.front.F * (i + 1)} y={positions.front.y + dims.front.F} width={dims.front.F} height={dims.front.H - 2 * dims.front.F} fill="rgba(255, 143, 0, 0.1)" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />);
      }
    }
    ...
    if (doubleFrame.top) {
      for (let i = 0; i < doubleFrame.count; i++) {
        elements.push(<rect key={`df-top-${i}`} x={positions.front.x + dims.front.F} y={positions.front.y + dims.front.F + dims.front.F * i} width={dims.front.W - 2 * dims.front.F} height={dims.front.F} fill="rgba(255, 143, 0, 0.1)" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />);
      }
    }
    ...
    if (doubleFrame.bottom) {
      for (let i = 0; i < doubleFrame.count; i++) {
        elements.push(<rect key={`df-bottom-${i}`} x={positions.front.x + dims.front.F} y={positions.front.y + dims.front.H - dims.front.F - dims.front.F * (i + 1)} width={dims.front.W - 2 * dims.front.F} height={dims.front.F} fill="rgba(255, 143, 0, 0.1)" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />);
      }
    }
    ...
    if (doubleFrame.center) {
      for (let i = 0; i < doubleFrame.count; i++) {
        ...
        elements.push(<rect key={`df-center-${i}`} x={positions.front.x + dims.front.W / 2 - dims.front.F / 2 + offsetFromCenter} y={positions.front.y + dims.front.F} width={dims.front.F} height={dims.front.H - 2 * dims.front.F} fill="rgba(255, 143, 0, 0.1)" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />);
      }
    }
  ...
  return (
    <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} width="210mm" height="297mm" className="w-full h-auto bg-white">
      {/* กรอบกระดาษ */}
      <rect x="8" y="8" width={viewBoxWidth - 16} height={viewBoxHeight - 16} fill="none" stroke="#000" strokeWidth="2" />
      <rect x="12" y="12" width={viewBoxWidth - 24} height={viewBoxHeight - 24} fill="none" stroke="#000" strokeWidth="0.5" />
      ...
      {/* Title */}
      <text x={viewBoxWidth / 2} y="80" textAnchor="middle" fontSize="40" fontWeight="bold">
        DOOR FRAME STRUCTURE DRAWING
      </text>

      {/* SIDE VIEW */}
      <g id="side-view">
        <text x={positions.side.x + dims.side.T / 2} y={positions.side.y - 60} textAnchor="middle" fontSize="32" fontWeight="bold">
          Side View
        </text>

        <rect x={positions.side.x} y={positions.side.y} width={dims.side.T} height={dims.side.H} fill="#fafafa" stroke="#000" strokeWidth="3" />

        {/* ผิวปิด */}
        <rect x={positions.side.x} y={positions.side.y} width={dims.side.S} height={dims.side.H} fill="#e8f5e9" stroke="#000" strokeWidth="1.2" />
        <rect x={positions.side.x + dims.side.T - dims.side.S} y={positions.side.y} width={dims.side.S} height={dims.side.H} fill="#e8f5e9" stroke="#000" strokeWidth="1.2" />

        {/* โครงรอบ + core */}
        <rect x={positions.side.x + dims.side.S} y={positions.side.y} width={(dims.side.T - 2 * dims.side.S) * 0.25} height={dims.side.H} fill="#fff3e0" stroke="#000" strokeWidth="1" />
        <rect x={positions.side.x + dims.side.T - dims.side.S - (dims.side.T - 2 * dims.side.S) * 0.25} y={positions.side.y} width={(dims.side.T - 2 * dims.side.S) * 0.25} height={dims.side.H} fill="#fff3e0" stroke="#000" strokeWidth="1" />
        <rect x={positions.side.x + dims.side.S + (dims.side.T - 2 * dims.side.S) * 0.25} y={positions.side.y} width={(dims.side.T - 2 * dims.side.S) * 0.5} height={dims.side.H} fill="#fce4ec" stroke="#000" strokeWidth="0.8" strokeDasharray="4,4" />
        ...
        {railPositions.map((pos, idx) => {
          ...
          return <rect key={`side-rail-${idx}`} x={positions.side.x + dims.side.S} y={railY - railH / 2} width={dims.side.T - 2 * dims.side.S} height={railH} fill="#ffe0b2" stroke="#000" strokeWidth="1" />;
        })}

        {(lockBlockLeft || lockBlockRight) && <rect x={positions.side.x + dims.side.S} y={positions.side.y + dims.side.H - lockBlockBottom * sideScale} width={dims.side.T - 2 * dims.side.S} height={LOCK_BLOCK_HEIGHT * sideScale} fill="none" stroke="#c62828" strokeWidth="1.4" strokeDasharray="6,4" />}
      ...
      {/* FRONT VIEW */}
      <g id="front-view">
        <text x={positions.front.x + dims.front.W / 2} y={positions.front.y - 60} textAnchor="middle" fontSize="32" fontWeight="bold">
          Front View
        </text>

        <rect x={positions.front.x} y={positions.front.y} width={dims.front.W} height={dims.front.H} fill="#fafafa" stroke="#000" strokeWidth="3" />

        {/* Main frame - Left stile */}
        <rect x={positions.front.x} y={positions.front.y} width={dims.front.F} height={dims.front.H} fill="#fff3e0" stroke="#000" strokeWidth="1.6" />
        {/* Main frame - Right stile */}
        <rect x={positions.front.x + dims.front.W - dims.front.F} y={positions.front.y} width={dims.front.F} height={dims.front.H} fill="#fff3e0" stroke="#000" strokeWidth="1.6" />

        {/* Main frame - Top rail */}
        <rect x={positions.front.x + dims.front.F} y={positions.front.y} width={dims.front.W - 2 * dims.front.F} height={dims.front.F} fill="#ffe0b2" stroke="#000" strokeWidth="1.6" />
        {/* Main frame - Bottom rail */}
        <rect x={positions.front.x + dims.front.F} y={positions.front.y + dims.front.H - dims.front.F} width={dims.front.W - 2 * dims.front.F} height={dims.front.F} fill="#ffe0b2" stroke="#000" strokeWidth="1.6" />
        ...
        {railPositions.map((pos, idx) => {
          ...
          return (
            <g key={`front-rail-${idx}`}>
              <rect x={positions.front.x + dims.front.F} y={railY - dims.front.R / 2} width={dims.front.W - 2 * dims.front.F} height={dims.front.R} fill="#ffe0b2" stroke="#000" strokeWidth="1.2" />
              <line x1={positions.front.x + dims.front.F} y1={railY - dims.front.R / 2} x2={positions.front.x + dims.front.W - dims.front.F} y2={railY + dims.front.R / 2} stroke="#d7ccc8" strokeWidth="0.6" />
            </g>
          );
        })}
        ...
        {railPositions.map((pos, idx) => (
          <g key={`front-ann-${idx}`}>
            <line x1={positions.front.x + dims.front.W + 200} y1={positions.front.y + dims.front.H - pos * frontScale} x2={positions.front.x + dims.front.W + 240} y2={positions.front.y + dims.front.H - pos * frontScale} stroke="#666" strokeWidth="0.8" />
            <text x={positions.front.x + dims.front.W + 260} y={positions.front.y + dims.front.H - pos * frontScale + 10} fontSize="18" fill="#666">
              {pos}
            </text>
          </g>
        ))}
...
const BottomInfoBar = ({ viewBoxWidth, viewBoxHeight, T, W, H, S, F, R, surfaceMaterial, frameType, hasDoubleFrame, doubleFrame, railSections, railPositions, lockBlockCount, currentFrame }) => {
  ...
  return (
    <g id="bottom-info">
      <rect x="20" y={viewBoxHeight - 145} width={viewBoxWidth - 40} height="130" fill="#f8fafc" stroke="#1e3a5f" strokeWidth="1.5" rx="3" />

      <g id="specs">
        <rect x="25" y={viewBoxHeight - 140} width="370" height="20" fill="#1e3a5f" rx="2" />
        <text x="35" y={viewBoxHeight - 126} fontSize="10" fontWeight="bold" fill="white">
          📋 SPECIFICATIONS
        </text>
        <rect x="25" y={viewBoxHeight - 118} width="370" height="100" fill="white" stroke="#e2e8f0" strokeWidth="0.5" />

        <text x="35" y={viewBoxHeight - 100} fontSize="8" fill="#64748b">
          Door Size:
        </text>
        <text x="35" y={viewBoxHeight - 88} fontSize="11" fontWeight="bold" fill="#1e293b">
          {T} × {W} × {H} mm
        </text>
        ...
        <text x="35" y={viewBoxHeight - 58} fontSize="9" fontWeight="600" fill="#059669">
        ...
        <text x="35" y={viewBoxHeight - 28} fontSize="9" fontWeight="600" fill="#d97706">
        ...
        <text x="200" y={viewBoxHeight - 88} fontSize="9" fontWeight="600" fill="#ea580c">
        ...
        <text x="200" y={viewBoxHeight - 58} fontSize="9" fontWeight="600" fill="#dc2626">
        ...
        {currentFrame.code && (
          <>
            ...
            <text x="200" y={viewBoxHeight - 28} fontSize="8" fontWeight="500" fill="#6366f1" fontFamily="monospace">
              {currentFrame.code}
            </text>
          </>
        )}
      ...
      <g id="legend">
        <rect x="405" y={viewBoxHeight - 140} width="280" height="20" fill="#1e3a5f" rx="2" />
        <text x="415" y={viewBoxHeight - 126} fontSize="10" fontWeight="bold" fill="white">
          🎨 LEGEND
        </text>
        <rect x="405" y={viewBoxHeight - 118} width="280" height="100" fill="white" stroke="#e2e8f0" strokeWidth="0.5" />
        ...
        {[
          ...
          <g key={`legend-${i}`}>
            <rect x={item.x} y={viewBoxHeight + item.y} width="16" height="10" fill={item.fill} stroke={item.stroke} strokeWidth="0.8" rx="1" strokeDasharray={item.dashed ? "2,1" : undefined} />
            <text x={item.x + 20} y={viewBoxHeight + item.y + 8} fontSize="7" fill="#374151">
              {item.label}
            </text>
          </g>
        ))}

        <line x1="415" y1={viewBoxHeight - 49} x2="431" y2={viewBoxHeight - 49} stroke="#333" strokeWidth="0.6" strokeDasharray="8,2,2,2" />
        <text x="435" y={viewBoxHeight - 46} fontSize="7" fill="#374151">
          Center Line
        </text>

        <line x1="525" y1={viewBoxHeight - 49} x2="541" y2={viewBoxHeight - 49} stroke="#333" strokeWidth="0.6" strokeDasharray="3,3" />
        <text x="545" y={viewBoxHeight - 46} fontSize="7" fill="#374151">
          Hidden Line
        </text>

        <line x1="415" y1={viewBoxHeight - 32} x2="431" y2={viewBoxHeight - 32} stroke="#333" strokeWidth="0.6" />
        <polygon points={`415,${viewBoxHeight - 32} 418,${viewBoxHeight - 34} 418,${viewBoxHeight - 30}`} fill="#333" />
        <polygon points={`431,${viewBoxHeight - 32} 428,${viewBoxHeight - 34} 428,${viewBoxHeight - 30}`} fill="#333" />
      ...
      <g id="title-block">
        <rect x={viewBoxWidth - 305} y={viewBoxHeight - 140} width="280" height="120" fill="white" stroke="#1e3a5f" strokeWidth="1.5" rx="2" />
        <rect x={viewBoxWidth - 305} y={viewBoxHeight - 140} width="280" height="28" fill="#1e3a5f" rx="2" />
        <rect x={viewBoxWidth - 305} y={viewBoxHeight - 115} width="280" height="3" fill="#1e3a5f" />
        <text x={viewBoxWidth - 165} y={viewBoxHeight - 121} textAnchor="middle" fontSize="13" fontWeight="bold" fill="white">
          DOOR FRAME ASSEMBLY
        </text>

        <rect x={viewBoxWidth - 300} y={viewBoxHeight - 107} width="270" height="22" fill="#f1f5f9" rx="1" />
        <text x={viewBoxWidth - 290} y={viewBoxHeight - 92} fontSize="8" fill="#64748b">
          Size:
        </text>
        <text x={viewBoxWidth - 165} y={viewBoxHeight - 91} textAnchor="middle" fontSize="14" fontWeight="bold" fill="#1e293b">
          {T} × {W} × {H} mm
        </text>

        <line x1={viewBoxWidth - 300} y1={viewBoxHeight - 82} x2={viewBoxWidth - 30} y2={viewBoxHeight - 82} stroke="#e2e8f0" strokeWidth="0.5" />
        <text x={viewBoxWidth - 290} y={viewBoxHeight - 68} fontSize="8" fill="#64748b">
          Material:
        </text>
        <text x={viewBoxWidth - 165} y={viewBoxHeight - 68} textAnchor="middle" fontSize="10" fontWeight="600" fill="#059669">
          {surfaceMaterial} + {frameType}
        </text>

        <line x1={viewBoxWidth - 300} y1={viewBoxHeight - 55} x2={viewBoxWidth - 30} y2={viewBoxHeight - 55} stroke="#e2e8f0" strokeWidth="0.5" />
        <line x1={viewBoxWidth - 165} y1={viewBoxHeight - 55} x2={viewBoxWidth - 165} y2={viewBoxHeight - 35} stroke="#e2e8f0" strokeWidth="0.5" />

        <text x={viewBoxWidth - 290} y={viewBoxHeight - 42} fontSize="8" fill="#64748b">
          Scale:
        </text>
        <text x={viewBoxWidth - 240} y={viewBoxHeight - 42} fontSize="10" fontWeight="600" fill="#1e293b">
          NTS
        </text>

        <text x={viewBoxWidth - 155} y={viewBoxHeight - 42} fontSize="8" fill="#64748b">
          Rev:
        </text>
        <text x={viewBoxWidth - 120} y={viewBoxHeight - 42} fontSize="10" fontWeight="600" fill="#1e293b">
          1.0
        </text>

        <rect x={viewBoxWidth - 305} y={viewBoxHeight - 35} width="280" height="15" fill="#f8fafc" />
        <line x1={viewBoxWidth - 300} y1={viewBoxHeight - 35} x2={viewBoxWidth - 30} y2={viewBoxHeight - 35} stroke="#e2e8f0" strokeWidth="0.5" />
        <text x={viewBoxWidth - 165} y={viewBoxHeight - 24} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#1e3a5f">
    ...
    <div className="flex flex-col items-center justify-start w-full h-full p-4 gap-4 overflow-auto bg-gradient-to-br from-blue-50 to-gray-100">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-1">🚪 Door Configuration System</h1>
          <p className="text-gray-600">ระบบออกแบบโครงประตู - C.H.H INDUSTRY CO., LTD.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-4">
          <div className="lg:col-span-2 space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <SectionCard text="1" title="สเปคลูกค้า" icon="📝" color="blue">
                <div className="space-y-3">
                  <QuickSelectWithInput label="ความหนา (T) mm" value={doorThickness} onChange={setDoorThickness} themeColor="#2563eb" />
                  <QuickSelectWithInput label="ความกว้าง (W mm)" value={doorWidth} onChange={setDoorWidth} themeColor="#2563eb" />
                  <QuickSelectWithInput label="ความสูง (H) mm" value={doorHeight} onChange={setDoorHeight} themeColor="#2563eb" />
                  <InfoBox color="blue" className="text-center">
                    <p>
                      สเปค: <span className="font-bold">{formatDimension(doorThickness, doorWidth, doorHeight)}</span> mm
                    </p>
                  </InfoBox>
                </div>
              </SectionCard>
              ...
              <SectionCard text="2" title="วัสดุปิดผิว" icon="🎨" color="green">
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">ประเภทวัสดุ</label>
                    <div className="grid grid-cols-2 gap-1">
                      {SURFACE_MATERIALS.map((mat) => (
                        <button key={mat.value} onClick={() => setSurfaceMaterial(mat.value)} className={`py-1.5 px-2 rounded text-xs font-medium transition-all ${surfaceMaterial === mat.value ? "bg-green-600 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          {mat.label}
                        </button>
                      ))}
                    </div>
                  ...
                  <QuickSelectWithInput label="ความหนา/แผ่น" value={surfaceThickness} onChange={setSurfaceThickness} themeColor="#16a34a" />

                  <InfoBox color="green">
                    <InfoRow label="วัสดุ:" value={<span className="text-green-700">{getMaterialLabel(SURFACE_MATERIALS, surfaceMaterial)}</span>} />
                    <InfoRow label="วัสดุปิดผิว:" value={`${surfaceThickness || 0} mm × 2 = ${(parseFloat(surfaceThickness) || 0) * 2} mm`} className="text-gray-600" />
                    <InfoRow label="กาว:" value={`${GLUE_THICKNESS} mm × 2 = ${GLUE_THICKNESS * 2} mm`} className="text-gray-600" />
                    <InfoRow label="รวมทั้งหมด:" value={`${results.totalSurfaceThickness} mm`} className="font-medium border-t border-green-200 pt-1 mt-1" />
                    <InfoRow label="ความหนาโครงที่ต้องการ:" value={<span className="text-green-700">{results.frameThickness} mm</span>} className="pt-1 border-t border-green-200 mt-1" />
                  </InfoBox>
                </div>
              </SectionCard>
              ...
              <SectionCard text="3" title="โครง (ERP)" icon="🪵" color="amber">
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-1">
                    {FRAME_TYPES.map((opt) => (
                      <button key={opt.value} onClick={() => setFrameType(opt.value)} className={`py-1.5 rounded text-xs font-medium transition-all ${frameType === opt.value ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">
                      ...
                      <span className="text-gray-400 font-normal ml-1">ยาว≥{doorHeight || 0}mm</span>
                    </label>
                    {frameType && frameSelection.frames.length > 0 ? (
                      <select value={selectedFrameCode} onChange={(e) => setSelectedFrameCode(e.target.value)} className="w-full px-2 py-1.5 border rounded text-xs focus:ring-2 focus:ring-amber-500">
                      ...
                      <InfoBox color="red">⚠️ {frameSelection.reason || `ไม่มีไม้ที่ใช้ได้สำหรับความหนา ${results.frameThickness}mm`}</InfoBox>
                    ) : (
                      <InfoBox color="gray">กรุณาเลือกประเภทไม้โครงก่อน</InfoBox>
                    )}
                  </div>

                  {frameType && frameSelection.frames.length > 0 && (
                    <InfoBox color={currentFrame.needSplice ? "purple" : currentFrame.planeAmount > 0 || currentFrame.isFlipped ? "orange" : "amber"}>
                      <InfoRow
                      ...
                      />
                      <InfoRow label="รหัส ERP:" value={<span className="font-mono text-[10px]">{selectedFrameCode}</span>} className="pt-1 border-t mt-1" />
                      ...
                      {currentFrame.planeAmount > 0 && (
                        <div className="text-orange-600 mt-1 pt-1 border-t flex items-center gap-1">
                          <span>🪚</span> <span>ต้องไสเนื้อออก {currentFrame.planeAmount} mm</span>
                        </div>
                      )}
                      {currentFrame.needSplice && (
                        <div className="text-purple-600 mt-1 pt-1 border-t">
                          <div className="flex items-center gap-1 font-medium">
                            <span>🔗</span> <span>ต่อไม้ {currentFrame.spliceCount} ท่อน</span>
                          </div>
                          <div className="text-[10px] mt-1 space-y-0.5">
                            <div>• ตำแหน่งต่อ: {currentFrame.splicePosition} mm จากปลาย</div>
                            <div>• เผื่อซ้อนทับ: {currentFrame.spliceOverlap} mm</div>
                            <div>• ความยาวรวม: {currentFrame.effectiveLength} mm</div>
                          </div>
                        </div>
                      )}
                    </InfoBox>
                  ...
                  <div className="space-y-2">
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">ด้านที่ต้องการเบิ้ลโครง</label>
                      <div className="grid grid-cols-3 gap-1">
                        <button onClick={() => handleToggleDoubleSide("top")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.top ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          บน
                        </button>
                        <button onClick={() => handleToggleDoubleSide("bottom")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.bottom ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          ล่าง
                        </button>
                        <button onClick={() => handleToggleDoubleSide("left")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.left ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          ซ้าย
                        </button>
                        <button onClick={() => handleToggleDoubleSide("center")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.center ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          กลาง
                        </button>
                        <button onClick={() => handleToggleDoubleSide("right")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.right ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          ขวา
                        </button>
                        <button onClick={() => handleToggleDoubleSide("all")} className={`py-1.5 rounded text-xs font-medium transition-all ${doubleFrameSides.all ? "bg-amber-600 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                          ทั้งหมด
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">จำนวนไม้เบิ้ลต่อด้าน</label>
                      <div className="flex items-center gap-1">
                        {[1, 2, 3].map((n) => (
                          <button key={n} onClick={() => setDoubleFrameCount(String(n))} className={`px-2 py-1 rounded text-xs font-medium transition-all ${numericDoubleCount === n ? "bg-amber-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                            {n}
                          </button>
                        ))}
                        <div className="flex-1">
                          <NumberInput value={numericDoubleCount} onChange={(v) => setDoubleFrameCount(String(v || 0))} className="w-full px-2 py-1.5 border rounded text-center text-xs font-bold" />
                        </div>
                    ...
                    {doubleConfigSummary && (
                      <InfoBox color="amber">
                        <div className="text-[11px] text-amber-800">{doubleConfigSummary}</div>
                      </InfoBox>
                    )}
                  </div>
                </div>
              ...
              <SectionCard text="4" title="ไม้ดามแนวนอน" icon="➖" color="orange">
                <InfoBox color="orange">
                  <InfoRow
                    ...
                    className="mb-1"
                  />
                  {doorHeight >= 2400 && <div className="text-orange-600 text-[10px] mb-1">⚡ ประตูสูงเกิน 2400mm → แบ่ง 4 ช่อง อัตโนมัติ</div>}
                  {results.railsAdjusted && <div className="text-amber-600 text-[10px] mb-1 p-1 bg-amber-50 rounded">🔄 ปรับตำแหน่งไม้ดามอัตโนมัติเพื่อหลบ Lock Block</div>}
                  <InfoRow
                    label="ขนาดไม้ดาม:"
                    value={
                      <span className="text-orange-700">
                    ...
                    className="mb-1"
                  />
                  <div className="text-gray-500 text-[10px]">(ใช้ไม้เดียวกับโครง)</div>
                    ...
                    {results.railPositions.map((pos, idx) => {
                      ...
                      return (
                        <InfoRow
                          ...
                          className={wasAdjusted ? "text-amber-600" : "text-orange-700"}
                        />
                      );
                    })}
                  </div>
            ...
            <SectionCard text="5" title="Lock Block (รองลูกบิด)" icon="🔒" color="red">
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">จำนวนต่อฝั่ง</label>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4].map((n) => (
                      <button key={n} onClick={() => setLockBlockPiecesPerSide(n)} className={`flex-1 py-2 rounded text-sm font-bold transition-all ${piecesPerSide === n ? "bg-red-500 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200"}`}>
                        {n} ชิ้น
                      </button>
                    ))}
                  </div>
                ...
                {(lockBlockLeft || lockBlockRight) && piecesPerSide > 0 && (
                  <InfoBox color="red">
                    <InfoRow
                      ...
                      value={
                        <span className="text-red-700">
                          {results.lockBlockCount} ชิ้น ({lockBlockLeft && lockBlockRight ? `ซ้าย ${piecesPerSide} + ขวา ${piecesPerSide}` : lockBlockLeft ? `ซ้าย ${piecesPerSide}` : `ขวา ${piecesPerSide}`})
                        </span>
                      }
                      className="mb-1"
                    />
                    <InfoRow
                      ...
                      className="mb-1"
                    />
                    <div className="text-gray-500 text-[10px] mb-2">(ใช้ไม้เดียวกับโครง)</div>
                    <div className="pt-2 border-t border-red-200 text-red-700 space-y-1">
                      <InfoRow label="ขอบบน:" value={`${results.lockBlockTop} mm จากพื้น`} />
                      <InfoRow label="กึ่งกลาง:" value={`${results.lockBlockPosition} mm จากพื้น`} />
                      <InfoRow label="ขอบล่าง:" value={`${results.lockBlockBottom} mm จากพื้น`} />
                    </div>
                  </InfoBox>
                )}
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4">
              <h3 className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">📋 สรุปโครงสร้าง</h3>
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className={`p-2 rounded-lg ${currentFrame.planeAmount > 0 || currentFrame.isFlipped ? "bg-orange-50" : "bg-amber-50"}`}>
                  <span className="text-gray-500 block">โครงไม้:</span>
                  <span className="font-bold text-amber-600">
                    {currentFrame.useThickness || "-"}×{currentFrame.useWidth || "-"} mm
                  </span>
                  {currentFrame.isFlipped && <span className="block text-[10px] text-orange-600">🔄 พลิกไม้</span>}
                  {currentFrame.planeAmount > 0 && <span className="block text-[10px] text-orange-600">🪚 ไส {currentFrame.planeAmount}mm</span>}
                </div>
              ...
              {doubleConfigSummary && <div className="mt-3 p-2 bg-amber-50 rounded-lg text-[11px] text-amber-800">{doubleConfigSummary}</div>}
              {selectedFrameCode && (
                <div className="mt-3 p-2 bg-blue-50 rounded-lg text-xs">
                  <div className="font-medium text-blue-800">รหัส ERP: {selectedFrameCode}</div>
                  <div className="text-blue-600 text-[10px]">{currentFrame.desc}</div>
                </div>
              )}
            </div>
                ...
                <div className="space-y-4">
                  {cuttingPlan.needSplice && (
                    <InfoBox color="purple">
                      ...
                      <div className="text-purple-600 space-y-0.5">
                        ...
                        <div className="text-[10px] text-purple-500 mt-1">💡 ใช้กาว + ตะปูยึดบริเวณรอยต่อ</div>
                      </div>
                    </InfoBox>
                  )}

                  <div className="grid grid-cols-4 gap-2 text-xs">
                    <StatCard value={cuttingPlan.totalStocks} label="ไม้ที่ใช้ (ท่อน)" bgColor="bg-indigo-50" textColor="text-indigo-600" />
                    <StatCard value={cuttingPlan.efficiency} label="ประสิทธิภาพ" bgColor="bg-green-50" textColor="text-green-600" />
                    <StatCard value={cuttingPlan.usedWithoutKerf} label="ใช้จริง (mm)" bgColor="bg-blue-50" textColor="text-blue-600" />
                    <StatCard value={cuttingPlan.totalWaste} label="เศษเหลือ (mm)" bgColor="bg-red-50" textColor="text-red-600" />
                  </div>
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    ...
                    <div className="divide-y">
                      {cuttingPlan.cutPieces.map((piece, idx) => (
                        <div key={idx} className={`flex items-center justify-between px-3 py-2 text-xs hover:bg-gray-50 ${piece.isSplice ? "bg-purple-50" : ""}`}>
                          <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded" style={{ backgroundColor: piece.color }}></div>
                            <span className="font-medium">{piece.name}</span>
                            {piece.isSplice && <span className="text-[9px] text-purple-600 bg-purple-100 px-1 rounded">ต่อ</span>}
                          </div>
                          <div className="flex items-center gap-4 text-gray-600">
                            <span>{piece.length} mm</span>
                            <span className="font-bold text-gray-800">×{piece.qty}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="p-3 space-y-2">
                      {cuttingPlan.stocks.map((stock, stockIdx) => (
                        <div key={stockIdx} className="space-y-1">
                          <div className="text-[10px] text-gray-500">ท่อนที่ {stockIdx + 1}</div>
                  ...
                  <div className="p-2 bg-gray-50 rounded-lg">
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span className={`font-bold ${getEfficiencyColor(cuttingPlan.efficiency).text}`}>{cuttingPlan.efficiency}%</span>
                    </div>
                    <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all ${getEfficiencyColor(cuttingPlan.efficiency).bg}`} style={{ width: `${cuttingPlan.efficiency}%` }} />
                    </div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden sticky top-4">
              <div className="bg-gradient-to-r from-slate-700 to-slate-800 px-4 py-2.5 flex justify-between items-center">
                ...
                {isDataComplete && (
                  <button onClick={() => setIsDrawingModalOpen(true)} className="bg-white/20 hover:bg-white/30 text-white px-3 py-1 rounded-lg text-xs font-medium flex items-center gap-1 transition-colors">
              ...
              <div className={`p-3 bg-gray-50 ${isDataComplete ? "cursor-pointer hover:bg-gray-100 transition-colors" : ""}`} onClick={() => isDataComplete && setIsDrawingModalOpen(true)}>
                {isDataComplete ? (
                  <>
                    <EngineeringDrawing results={results} />
                    <div className="text-center mt-2 text-xs text-gray-400">💡 คลิกที่รูปเพื่อขยาย</div>
                  </>
                ) : (
                  <EmptyState
                    icon={
                  ...
                  >
                    <div className="mt-4 flex gap-2 text-xs">
                      <span className={`px-2 py-1 rounded ${doorThickness ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>T: {doorThickness || "—"}</span>
                      <span className={`px-2 py-1 rounded ${doorWidth ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>W: {doorWidth || "—"}</span>
                      <span className={`px-2 py-1 rounded ${doorHeight ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>H: {doorHeight || "—"}</span>
                    </div>
                  </EmptyState>
                )}
              </div>
