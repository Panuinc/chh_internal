...
const getEfficiencyColor = (efficiency) => {
  ...
  if (val >= 80) return THEME_COLORS.success;
  if (val >= 60) return THEME_COLORS.warning;
...
const DimLine = ({ x1, y1, x2, y2, value, offset = 25, vertical = false, color = THEME_COLORS.foreground, fontSize = 9, unit = "" }) => {
  ...
  if (vertical) {
    ...
    return (
      <g className="dimension">
        ...
        <rect x={lineX - textWidth / 2} y={midY - 6} width={textWidth} height="12" fill={THEME_COLORS.background} />
  ...
  return (
    <g className="dimension">
      ...
      <rect x={midX - textWidth / 2} y={lineY - 6} width={textWidth} height="12" fill={THEME_COLORS.background} />
...
const useCuttingPlan = (results, currentFrame) => {
  return useMemo(() => {
    ...
    if (needSplice && stileLength > stockLength) {
      ...
      addPiece("โครงตั้ง (ท่อน 1)", pieceLength, 2, THEME_COLORS.secondary, true);
      addPiece("โครงตั้ง (ท่อน 2)", pieceLength, 2, THEME_COLORS.warning, true);
    } else {
      addPiece("โครงตั้ง", stileLength, 2, THEME_COLORS.secondary);
    }

    addPiece("โครงนอน", W - 2 * F, 2, THEME_COLORS.primary);
    ...
    if (doubleFrame && doubleFrame.hasAny && doubleFrame.count > 0) {
      ...
      const addDoubleVertical = (label, enabled) => {
        ...
        if (needSplice && clearHeight > stockLength) {
          ...
          addPiece(label + " (ท่อน 1)", pieceLength, count, THEME_COLORS.warning, true);
          addPiece(label + " (ท่อน 2)", pieceLength, count, THEME_COLORS.secondary, true);
        } else {
          addPiece(label, clearHeight, count, THEME_COLORS.warning);
        }
      };

      const addDoubleHorizontal = (label, enabled) => {
        if (!enabled) return;
        addPiece(label, clearWidth, count, THEME_COLORS.secondary);
      };
    ...
    if (railCount > 0) {
      addPiece("ไม้ดาม", clearWidth, railCount, THEME_COLORS.primary);
    }
      ...
      <g id="legend">
        ...
        {[
          ...
          <g key={`legend-${i}`}>
            ...
            <text x={item.x + 20} y={viewBoxHeight + item.y + 8} fontSize="7" fill={THEME_COLORS.foreground}>
              {item.label}
            </text>
          </g>
        ))}
...
const EngineeringDrawing = ({ results }) => {
  ...
  const renderDoubleFrames = () => {
    ...
    const elements = [];
    const strokeColor = THEME_COLORS.warning;
  ...
  return (
    <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} width="210mm" height="297mm" className="w-full h-auto" style={{ backgroundColor: THEME_COLORS.background }}>
      <rect x="8" y="8" width={viewBoxWidth - 16} height={viewBoxHeight - 16} fill="none" stroke={THEME_COLORS.foreground} strokeWidth="2" />
      <rect x="12" y="12" width={viewBoxWidth - 24} height={viewBoxHeight - 24} fill="none" stroke={THEME_COLORS.foreground} strokeWidth="0.5" />

      <g id="grid-ref" fontSize="20" fill={THEME_COLORS.default}>
      ...
      <text x={viewBoxWidth / 2} y="80" textAnchor="middle" fontSize="40" fontWeight="bold" fill={THEME_COLORS.foreground}>
        ...
        <div className="grid lg:grid-cols-5 gap-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <SectionCard text="1" title="สเปคลูกค้า" icon="📝" color="primary">
                <div className="space-y-3">
                  <QuickSelectWithInput label="ความหนา (T) mm" value={doorThickness} onChange={setDoorThickness} />
                  <QuickSelectWithInput label="ความกว้าง (W mm)" value={doorWidth} onChange={setDoorWidth} />
                  <QuickSelectWithInput label="ความสูง (H) mm" value={doorHeight} onChange={setDoorHeight} />
              ...
              <SectionCard text="2" title="วัสดุปิดผิว" icon="🎨" color="green">
                <div className="space-y-3">
                  ...
                  <InfoBox color="green">
                    ...
                    <InfoRow label="กาว:" value={`${GLUE_THICKNESS} mm × 2 = ${GLUE_THICKNESS * 2} mm`} className="mt-1" />
                    <InfoRow label="รวมทั้งหมด:" value={`${results.totalSurfaceThickness} mm`} className="font-medium border-t pt-1 mt-1" />
                    <InfoRow label="ความหนาโครงที่ต้องการ:" value={<span style={{ color: THEME_COLORS.success }}>{results.frameThickness} mm</span>} className="pt-1 border-t mt-1" />
                  </InfoBox>
                </div>
              </SectionCard>
                ...
                <div className="space-y-3">
                  ...
                  {frameType && frameSelection.frames.length > 0 && (
                    <InfoBox color={currentFrame.needSplice ? "purple" : currentFrame.planeAmount > 0 || currentFrame.isFlipped ? "orange" : "amber"}>
                      ...
                      <InfoRow
                        label="รหัส ERP:"
                        value={
                          <span className="font-mono text-[10px]" style={{ color: THEME_COLORS.foreground }}>
                            {selectedFrameCode}
                          </span>
                        }
                      ...
                      {currentFrame.isFlipped && (
                        <div className="mt-1 pt-1 border-t flex items-center gap-1 text-xs" style={{ color: THEME_COLORS.secondary }}>
                          <span>🔄</span>
                          <span>
                      ...
                      {currentFrame.planeAmount > 0 && (
                        <div className="mt-1 pt-1 border-t flex items-center gap-1 text-xs" style={{ color: THEME_COLORS.secondary }}>
                          <span>🪚</span>
                          <span>ต้องไสเนื้อออก {currentFrame.planeAmount} mm</span>
                        </div>
                      )}
                      {currentFrame.needSplice && (
                        <div className="mt-1 pt-1 border-t text-xs" style={{ color: THEME_COLORS.primary }}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        {[1, 2, 3].map((n) => {
                          const active = numericDoubleCount === n;
                          return (
                            <button
                              key={n}
                    ...
                    {doubleConfigSummary && (
                      <InfoBox color="amber">
                        <div className="text-[11px]" style={{ color: THEME_COLORS.secondary }}>
                          {doubleConfigSummary}
                        </div>
                      </InfoBox>
                    )}
              ...
              <SectionCard text="4" title="ไม้ดามแนวนอน" icon="➖" color="orange">
                <InfoBox color="orange">
                  ...
                  {doorHeight && parseFloat(doorHeight) >= 2400 && (
                    <div className="text-[10px] mb-1" style={{ color: THEME_COLORS.secondary }}>
                  ...
                  <InfoRow
                    ...
                    className="mb-1"
                  />
                  <div className="text-[10px]" style={{ color: THEME_COLORS.foreground, opacity: 0.8 }}>
                    (ใช้ไม้เดียวกับโครง)
                  </div>
                ...
                {(lockBlockLeft || lockBlockRight) && piecesPerSide > 0 && (
                  <InfoBox color="red">
                    <InfoRow
                      label="จำนวนรวม:"
                      value={
                        <span style={{ color: THEME_COLORS.danger }}>
                    ...
                    <InfoRow
                      ...
                      className="mb-1"
                    />
                    <div className="text-[10px] mb-2" style={{ color: THEME_COLORS.foreground, opacity: 0.8 }}>
                      (ใช้ไม้เดียวกับโครง)
                    </div>
                    <div className="pt-2 border-t text-xs" style={{ color: THEME_COLORS.danger }}>
          ...
          <div className="lg:col-span-3">
            <div className="rounded-2xl shadow-lg overflow-hidden sticky top-4" style={{ backgroundColor: THEME_COLORS.background }}>
              ...
              <div className="p-3" style={{ backgroundColor: `${THEME_COLORS.default}11` }} onClick={() => isDataComplete && setIsDrawingModalOpen(true)}>
                {isDataComplete ? (
                  <>
