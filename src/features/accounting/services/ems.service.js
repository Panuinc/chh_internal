...
async function getJWTToken() {
  ...
  const maskedKey =
    ...
    "..." +
    cleanToken.substring(cleanToken.length - 10);
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get JWT token: ${response.status} - ${responseText}`,
    );
  }
...
export async function trackEMS(barcode) {
  ...
  if (!emsPattern.test(barcode.trim())) {
    throw new Error(
      "Invalid EMS barcode format. Expected format: EN123456789TH",
    );
  }
  ...
  try {
    ...
    const responseText = await response.text();
    logger.info("Track API response", {
      status: response.status,
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        if (!retryResponse.ok) {
          throw new Error(
            `Track API error after retry: ${retryResponse.status}`,
          );
        }
    ...
    if (error.message?.includes("fetch failed")) {
      ...
      throw new Error(
        "Unable to connect to Thailand Post API. Please check your internet connection.",
      );
    }
