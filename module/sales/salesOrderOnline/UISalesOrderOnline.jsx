...
function DashboardDetailModal({
    ...
    return orders.filter((order) => {
      ...
      if (filterType === "unit") {
        return lines.some(
          (line) =>
            line.lineType === "Item" &&
            (line.unitOfMeasureCode || "ชิ้น") === filterValue,
        );
      }
      if (filterType === "product") {
        return lines.some(
          (line) => line.lineType === "Item" && line.itemNumber === filterValue,
        );
      }
      return false;
    });
  ...
  const summaryData = useMemo(() => {
    ...
    filteredOrders.forEach((order) => {
      ...
      lines.forEach((line) => {
        ...
        if (
          ...
          totalAmount += line.amountIncludingTax || 0;
          productSet.add(line.itemNumber);
        }
        if (filterType === "product" && line.itemNumber === filterValue) {
          totalQty += line.quantity || 0;
          totalAmount += line.amountIncludingTax || 0;
        }
      });
    ...
    return { totalQty, totalAmount, productCount: productSet.size };
  }, [filteredOrders, filterType, filterValue]);
      ...
      <ModalContent>
        <ModalHeader className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-4 text-xs text-foreground/60">
            ...
            <span>รวม {summaryData.totalQty.toLocaleString()} ชิ้น</span>
            <span>{formatCurrency(summaryData.totalAmount)} บาท</span>
            {filterType === "unit" && (
              <span>{summaryData.productCount} รายการสินค้า</span>
            )}
          </div>
        </ModalHeader>
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div className="flex flex-col gap-2 max-h-[60vh] overflow-auto">
              {filteredOrders.map((order, idx) => {
                const relevantLines = (order.salesOrderLines || []).filter(
                  (line) => {
                    if (line.lineType !== "Item") return false;
                    if (filterType === "unit") {
                      return (line.unitOfMeasureCode || "ชิ้น") === filterValue;
                    }
                    return line.itemNumber === filterValue;
                  },
                );

                const orderQty = relevantLines.reduce(
                  (sum, l) => sum + (l.quantity || 0),
                  0,
                );
                const orderAmount = relevantLines.reduce(
                  (sum, l) => sum + (l.amountIncludingTax || 0),
                  0,
                );
                ...
                return (
                  <div
                    ...
                    className="flex flex-col bg-default/30 rounded-lg p-3 border border-default/50"
                  >
                    ...
                    <div className="flex flex-col gap-1 pl-4 border-l-2 border-primary/20">
                      {relevantLines.map((line, lineIdx) => (
                        <div
                          key={lineIdx}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3 ml-2">
                            <span className="text-foreground/50">
                              {line.quantity} {line.unitOfMeasureCode || "ชิ้น"}
                            </span>
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-default/30 text-xs">
                      <span className="text-foreground/50">
                        รวมใน order นี้
                      </span>
...
function SalesDashboard({ orders, onSelectUnit, onSelectProduct }) {
  const { unitSummary, groupedProducts } = useMemo(() => {
    const productMap = {};
    const unitMap = {};
    ...
    orders.forEach((order) => {
      ...
      lines.forEach((line) => {
        ...
        const uom = line.unitOfMeasureCode || "ชิ้น";
        ...
        unitMap[uom].totalQty += line.quantity || 0;
        unitMap[uom].totalAmount += line.amountIncludingTax || 0;

        // Group products by unit
        const key = `${uom}-${line.itemNumber}`;
        if (!productMap[key]) {
          productMap[key] = {
          ...
          unitMap[uom].productCount++;
        }
        productMap[key].totalQty += line.quantity || 0;
        productMap[key].totalAmount += line.amountIncludingTax || 0;
      });
    });

    // Sort units by totalQty desc
    const sortedUnits = Object.values(unitMap).sort(
    ...
    sortedUnits.forEach((unit) => {
      grouped[unit.unitOfMeasure] = Object.values(productMap)
        .filter((p) => p.unitOfMeasure === unit.unitOfMeasure)
        .sort((a, b) => b.totalQty - a.totalQty);
    });

    return { unitSummary: sortedUnits, groupedProducts: grouped };
  }, [orders]);

  const totalQtyAll = unitSummary.reduce((sum, u) => sum + u.totalQty, 0);
  const totalAmountAll = unitSummary.reduce((sum, u) => sum + u.totalAmount, 0);

  if (unitSummary.length === 0) return null;

  return (
    <div className="flex flex-col items-center justify-start w-full h-full p-2 gap-2 overflow-auto border-2 border-foreground border-dashed">
      {/* Unit of Measure Summary */}
      <div className="flex flex-col items-center justify-start w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
        <div className="flex items-center justify-between w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
          <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
            <BarChart3 />
            <div className="flex items-center justify-start w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
              สรุปยอดขายตามหน่วยนับ
            </div>
          </div>
          <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
            <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
              <Package />
              {unitSummary.length} หน่วยนับ
            </div>
            <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
              <ShoppingCart size={14} />
              {totalQtyAll.toLocaleString()} ชิ้น
            </div>
            <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
              <TrendingUp size={14} />
              {formatCurrency(totalAmountAll)} บาท
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
          {unitSummary.map((unit, idx) => (
            <div
              ...
              className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed bg-default rounded-xl"
            >
              <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                  {idx + 1}
              ...
              <div className="flex flex-col items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                <div className="flex items-center justify-between w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                  <span className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                    สินค้า
                  </span>
                  <span className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                    {unit.productCount} รายการ
                  </span>
                </div>
                <div className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
                  <span className="flex items-center justify-center w-full h-full p-2 gap-2 border-2 border-foreground border-dashed">
      ...
      {unitSummary.map((unit) => (
        <div
          key={unit.unitOfMeasure}
          className="flex flex-col bg-default/20 rounded-xl border border-default/50 p-4 gap-3"
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-foreground/50">
                ({groupedProducts[unit.unitOfMeasure]?.length || 0} รายการ)
              </span>
            </div>
            <div className="flex items-center gap-3 text-xs text-foreground/60">
              <span>
                รวม {unit.totalQty.toLocaleString()} {unit.unitOfMeasure}
              </span>
              <span>{formatCurrency(unit.totalAmount)} บาท</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
            {groupedProducts[unit.unitOfMeasure]?.map((product, idx) => (
              <div
                key={product.itemNumber}
                onClick={() => onSelectProduct?.(product)}
                className="flex items-center gap-2 bg-background rounded-lg p-2 border border-default/30 hover:border-secondary/50 hover:bg-secondary/5 transition-all cursor-pointer"
              >
                <div className="flex items-center justify-center w-5 h-5 rounded-full bg-secondary/10 text-secondary text-[10px] font-bold">
                  {idx + 1}
                </div>
...
export default function UISalesOrderOnline({
  ...
  const handleSelectUnit = useCallback(
    (unit) => {
      setDashboardFilter({
        ...
        title: `รายการขาย - หน่วยนับ: ${unit.unitOfMeasure}`,
      });
      openDashboardDetail();
    },
    [openDashboardDetail],
  ...
  const handleSelectProduct = useCallback(
    (product) => {
      setDashboardFilter({
        ...
        title: `รายการขาย - สินค้า: ${product.description}`,
      });
      openDashboardDetail();
    },
    [openDashboardDetail],
  ...
  const handleCloseDashboardDetail = useCallback(() => {
    closeDashboardDetail();
    setDashboardFilter({ type: null, value: null, title: "" });
  }, [closeDashboardDetail]);
  ...
  return (
    <div className="flex flex-col xl:flex-row items-center justify-center w-full h-full overflow-hidden">
      ...
      <DashboardDetailModal
        isOpen={isDashboardDetailOpen}
        onClose={handleCloseDashboardDetail}
        title={dashboardFilter.title}
        orders={orders}
        filterType={dashboardFilter.type}
        filterValue={dashboardFilter.value}
      />
    </div>
  );
}
