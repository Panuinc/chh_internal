...
export async function generatePackingSlipZPL(
  ...
  const itemPadding = mm(1);
  const maxCharsPerLine = 35;

  let itemY = itemStartY;
  let itemIndex = 0;

  while (itemIndex < items.length && itemY < itemEndY) {
    const line = items[itemIndex];
    const num = itemIndex + 1;
    const qty = line.quantity || 0;
    const desc = sanitizeText(line.description || "", 100);

    const descLines = wrapText(desc, maxCharsPerLine);
    const rowHeight = descLines.length * singleLineH + itemPadding;

    if (itemY + rowHeight > itemEndY) {
    ...
    zpl += `^FO${colItemX + mm(2)},${itemY}^A0N,28,28^FD${num}^FS`;
    ...
    zpl += `^FO${colQtyX + mm(4)},${itemY}^A0N,28,28^FD${qty}^FS`;

    itemY += rowHeight;

    zpl += `^FO${M + mm(1)},${itemY - mm(0.5)}^GB${PW - mm(2)},1,1^FS`;
