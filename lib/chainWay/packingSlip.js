...
import { mmToDots, sanitizeText, getItemLines } from "./utils.js";
...
export async function generatePackingSlipZPL(
  ...
  const currentItem = items[piece - 1] || items[0];
  ...
  if (compName) zpl += `^FO${valueX},${y}${compName.command}^FS`; // แก้จาก valueX + mm(12) เป็น valueX
  ...
  // ========================================
  // BARCODE SECTION (20mm) - 90x20mm, Center
  // ========================================
  const barcodeTop = headerBottom;
  const barcodeH = mm(SECTION.BARCODE); // 20mm
  const barcodeBoxW = PW; // 90mm

  // คำนวณขนาด Barcode ให้พอดีช่อง
  // Barcode height: 12mm (เหลือ 8mm สำหรับ text + padding)
  const barcodeHeight = mm(12);
  // Text height ประมาณ 4mm
  const textHeight = mm(4);
  // Total content height = 12 + 4 = 16mm
  const totalContentH = barcodeHeight + textHeight;

  // คำนวณ Y ให้อยู่กึ่งกลางแนวตั้ง
  const barcodeY = barcodeTop + (barcodeH - totalContentH) / 2;

  // คำนวณ X ให้อยู่กึ่งกลางแนวนอน
  // CODE128: ประมาณ 11 modules/char + start/stop ~35 modules
  // module width = 2, ประมาณความกว้าง = (chars*11 + 35) * 2
  ...
  // ^BY = set bar width (module width)
  // ^BCN,height,print_line(Y=yes),check_digit,mode
  ...
  const itemEndY = footerTop - mm(1);
  const itemRowH = mm(5);

  let itemY = itemStartY;
  let itemIndex = 0;

  while (itemIndex < items.length && itemY + itemRowH < itemEndY) {
    const line = items[itemIndex];
    const num = itemIndex + 1;
    const qty = line.quantity || 0;
    const desc = sanitizeText(line.description || "", 40);

    zpl += `^FO${colItemX + mm(2)},${itemY}^A0N,30,30^FD${num}^FS`;

    const descG = await textToGraphic(desc, { fontSize: 28, maxWidth: mm(55) });
    if (descG) zpl += `^FO${colDescX},${itemY}${descG.command}^FS`;

    zpl += `^FO${colQtyX + mm(4)},${itemY}^A0N,30,30^FD${qty}^FS`;

    itemY += itemRowH;
    zpl += `^FO${M + mm(1)},${itemY - mm(1)}^GB${PW - mm(2)},1,1^FS`;
  ...
  if (itemIndex < items.length) {
    ...
    if (moreG) zpl += `^FO${colDescX},${itemY}${moreG.command}^FS`;
  }
