...
export async function buildThaiQRLabel(options) {
  ...
  const nameGraphic = await textToGraphic(displayName, { fontSize: 32, maxWidth: usableWidth - 220 });
  const name2Graphic = displayName2
    ? await textToGraphic(displayName2, { fontSize: 26, maxWidth: usableWidth - 220 })
    : null;
...
export async function buildThaiLabel(options) {
  ...
  const nameGraphic = await textToGraphic(displayName, { fontSize: 28, maxWidth: usableWidth });
  const name2Graphic = displayName2
...
export async function buildThaiRFIDLabel(options) {
  ...
  const nameGraphic = await textToGraphic(displayName, { fontSize: 32, maxWidth: usableWidth - 200 });
  const name2Graphic = displayName2
    ? await textToGraphic(displayName2, { fontSize: 26, maxWidth: usableWidth - 200 })
    : null;
