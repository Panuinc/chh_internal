...
export async function buildThaiRFIDLabel(options) {
  ...
  const row1Y = PAD.top;
  const itemFontSize = itemNumber.length > 15 ? 48 : 56;
  const maxItemChars = itemNumber.length > 15 ? 26 : 22;
  const itemText = sanitizeText(itemNumber, maxItemChars);

  const itemTextWidth = estimateTextWidth(itemText, itemFontSize);
  const itemX = Math.max(PAD.left, Math.floor((w - itemTextWidth) / 2));

  zpl += `^FO${itemX},${row1Y}^A0N,${itemFontSize},${itemFontSize}^FD${itemText}^FS`;

  if (totalQuantity > 1) {
    const sequenceText = `(${sequenceNumber}/${totalQuantity})`;
    const seqFontSize = 40;
    const seqWidth = mmToDots(18);
    const seqX = w - seqWidth;
  ...
  if (nameGraphic) {
    ...
    zpl += `^FO${nameX},${row2Y}${nameGraphic.command}^FS`;
  } else {
    ...
    zpl += `^FO${fallbackX},${row2Y}^A0N,36,36^FD${fallbackText}^FS`;
  }
