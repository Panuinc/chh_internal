/**
 * ZPL Builder for RFID Printer (Chainway CP30)
 * รองรับ: Barcode, QR Code, RFID Tag, Thai Text
 */
...
const DEFAULT_LABEL = { width: 100, height: 80 }; // mm
const DOTS_PER_MM = 11.8; // 300 DPI
...
// ============================================
// Thai Text to Graphic (สำหรับภาษาไทย)
// ============================================

/**
 * แปลงข้อความภาษาไทยเป็น ZPL Graphic
 * ต้องใช้ canvas library บน server
 */
export async function textToGraphic(text, options = {}) {
  ...
  try {
    ...
    measureCtx.font = `${fontSize}px Arial, Tahoma, sans-serif`;
...
/**
 * สร้าง Label พื้นฐาน (Barcode + Text)
 */
...
export async function buildThaiLabel(options) {
  ...
  // Item Number (English font OK)
  zpl += `^FO${margin},50^A0N,70,70^FD${sanitizeText(itemNumber)}^FS`;
  
  // Thai text as graphic
  if (nameGraphic) {
