...
const { dotsPerMm, padding: PADDING } = ZPL_CONFIG;

const PAD = {
  top: Math.round(PADDING.top * dotsPerMm),
  bottom: Math.round(PADDING.bottom * dotsPerMm),
  left: Math.round(PADDING.left * dotsPerMm),
  right: Math.round(PADDING.right * dotsPerMm),
};
...
export async function buildThaiQRLabel(options) {
  ...
  zpl += `^FO${PAD.left},${PAD.top}^A0N,45,45^FD${sanitizeText(
    itemNumber,
    30
  )}^FS`;
  ...
  if (nameGraphic) {
    ...
    zpl += `^FO${PAD.left},${h - PAD.bottom - 90}^A0N,38,38^FD${sanitizeText(
      displayName,
      40
    )}^FS`;
  }
  ...
  if (name2Graphic) {
    ...
    zpl += `^FO${PAD.left},${h - PAD.bottom - 45}^A0N,32,32^FD${sanitizeText(
      displayName2,
      40
    )}^FS`;
  }
  ...
  const qrSize = 200;
  zpl += `^FO${w - PAD.right - qrSize},${
    PAD.top + Math.round((usableHeight - qrSize) / 2)
  }^BQN,2,6^FDQA,${qrData || itemNumber}^FS`;
...
export async function buildThaiLabel(options) {
  ...
  zpl += `^FO${PAD.left},${PAD.top}^A0N,40,40^FD${sanitizeText(
    itemNumber,
    35
  )}^FS`;

  zpl += `^FO${PAD.left},${PAD.top + 50}^BY2,2,90^BCN,,Y,N^FD${
    barcodeData || itemNumber
  }^FS`;

  if (nameGraphic) {
    zpl += `^FO${PAD.left},${h - PAD.bottom - 70}${nameGraphic.command}^FS`;
  } else {
    zpl += `^FO${PAD.left},${h - PAD.bottom - 70}^A0N,32,32^FD${sanitizeText(
      displayName,
      45
    )}^FS`;
  }
  ...
  if (name2Graphic) {
    ...
    zpl += `^FO${PAD.left},${h - PAD.bottom - 35}^A0N,28,28^FD${sanitizeText(
      displayName2,
      45
    )}^FS`;
  }
...
export async function buildThaiRFIDLabel(options) {
  ...
  zpl += `^FO${w - PAD.right - 180},${
    PAD.top + Math.round((usableHeight - 85) / 2)
  }^A0N,85,85^FDRFID^FS`;
